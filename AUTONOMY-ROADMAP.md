...
| Scope and book work | Live catalog and invoice adapter ready, secrets/events not connected | Stripe live product and immutable $7,500/$5,000 Prices match `operations/stripe-catalog.json`; private bearer-protected invoice route is idempotent and uses Stripe-hosted invoicing | Managed Stripe key, operator token, accepted-scope caller, signed webhook consumer, and production event store |
